...
if (!$stmt) {
    ...
    $appointment_id = $status = $appointment_date = $appointment_time = $complaint = $service_type = $remarks = 'N/A';
} else {
    ...
    if (!$stmt->execute()) {
        ...
        if ($result === false) {
            ...
            $appointment_id = $status = $appointment_date = $appointment_time = $service_type = $complaint = $remarks = 'N/A';
        } else {
            ...
            if ($appointment = $result->fetch_assoc()) {
                ...
                $service_type = htmlspecialchars($appointment['service_type']);
